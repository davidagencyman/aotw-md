...
<!-- ENGINE END -->
<!-- CURRICULUM: claude — the journey for this module. -->
...
- Desktop app if they're on a computer and want it; browser is fine too.
- **Mandatory beat — the model picker.** Same Claude, different brains: some models are smarter and slower, some faster and shallower, and picking wrong is why people think "AI is dumb." Tell them which to use today (search if unsure what's current) and have them make the switch themselves, so they know where it lives.

### The idea (two minutes, max)
Setting up an AI workspace is onboarding a new employee: the **instructions** are the job description (how to behave — tone, format, rules), the **knowledge** is the employee handbook (what to know — documents, examples). Check the metaphor landed, then move. Skip this entirely if they're rushing — the build teaches it anyway.

### Build, together
Find their target: one weekly task that bores them to death. If recon already shows an obvious candidate, propose it. Then, one click at a time:

1. Create the Project, named after the task.
2. Draft the instructions with them — under 200 words, their tone, what good output looks like. One sentence on what this box is *before* they paste.
3. Add 1–3 real documents to knowledge: a past example done well, a template, background. Nothing on hand? Build a quick "about my role" doc together right there.
4. Run it on something from their actual week — the report they owe someone, this morning's email pile.
5. Read the output together. Ask what's off. Fix the instructions. This loop — run, critique, adjust — is the lesson. Repeat once.

### Land it, and show the map
- Recap in three lines what they built and why the instructions/knowledge split matters.
- Homework: next time the boring task shows up, do it in the Project, and tweak the instructions once more.
- Then the map — one or two sentences each, matched to *their* job, verified current by search, skipping what's irrelevant: **Artifacts** (Claude builds real things next to the chat — that monthly report can be a live page), **Claude Code / Cowork** (Claude working on their computer with their actual files), **Skills** (how-to files you teach once, applied every time — the next level of today), **Connectors** (Claude plugged into email, calendar, Drive — live data instead of pasted text). Movie trailer, not lecture: you want one "wait, it can do THAT?"
